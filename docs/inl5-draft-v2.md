...
The thesis sits at the intersection of two questions, and they pull in slightly different directions. One is the obvious benchmarking question — does the LLM-based single-token approach actually work, in the sense practitioners care about, against the methods we already have? Answering that is mostly a matter of running the experiments cleanly. The other is the one prior work has mostly skirted around: when an LLM tells you *why* it made a classification, is it telling you the truth, or is it telling you something plausible-sounding that doesn't have much to do with how the decision was actually made? That's a faithfulness question, and answering it well needs a different kind of evidence than the first one does.

> The thesis pursues two specific research questions. RQ1 asks how an LLM-based single-token classifier compares to established baselines such as DetectGPT and fine-tuned RoBERTa on standard AI-text detection benchmarks, and RQ2 asks whether the classification decision can be decomposed into human-interpretable explanations and, if so, how faithful those explanations turn out to be to the model's actual decision mechanism.

The first of those is the question §4.1 reports numbers against; the second is the question §4.3 reports an ablation against. The way they interact — the gradual realisation that an answer to RQ1 isn't worth much without an answer to RQ2 — is what shapes the rest of the thesis.
...
The pipeline starts with policy induction and ends with a small log-probability calibration layer, and the two steps are doing almost nothing in common with each other. Induction is essentially a writing exercise. A proposer LLM gets shown a handful of labelled training examples — usually somewhere between ten and twenty — and is asked, in plain English, to write down what it thinks distinguishes the AI ones from the human ones. The output isn't a structured rule schema, it's whatever roughly twenty lines of prose the model produces when you give it that prompt. We score the candidate against F0.5 on a held-out training subset (a hundred examples is enough, five hundred is plenty), and when it gets things wrong we feed those examples back and ask for a revised version; revisions only stick if F0.5 actually moves up, and the loop terminates either when improvements plateau (operationally, when the moving Δ stays under 0.005 across the last three accepted edits) or at the thirty-iteration budget, whichever shows up first. By the end, there is exactly one frozen policy string — the artefact going into the next step. Calibration starts there: with the policy installed as the classifier's system prompt, the classifier runs over training data and we record the log-probabilities of the first generated token for the relevant label tokens, including the third "other" outcome that catches refusals and hedges. On those features we fit a small calibration model — either a single threshold on `logp(yes) − logp(no)` or a logistic regression over the three log-probabilities — on the validation split to maximise F0.5, and that fitted layer plus the frozen policy is what we ultimately report performance for on the held-out test split.
...
Two tests per text, both at the whole-text level. The first is a same-policy consistency check: take the same text and the same policy, sample five times at temperature zero, and confirm you get identical outputs every time. (If you don't, there's non-determinism somewhere in the stack and everything downstream is suspect.) The second is the actual faithfulness test — a policy ablation. The same text gets classified under three different policies: the best policy from induction, an empty system prompt, and an adversarially-inverted policy that tells the classifier to assume everything in front of it was written by a human. If the predicted label and the log-probability margin shift in the direction the policy content commands, the policy is doing real work. If they don't shift much — if the model classifies the same way no matter what the prompt says — then the policy is a figurehead and any explanation built on top of it is suspect. Sentence-level and feature-level ablations are nicer to have but were deferred past the Inl. 5 deadline; only the whole-text protocol is reported here.
...
The question this thesis set out to answer was, more or less: can an LLM be made to detect AI-generated text both reliably and explainably, with neither half of that conjunction shortchanging the other? The honest answer the evidence here supports is *yes, qualified*. With a small open-weights model — Gemma 4 E4B — and a roughly 150-word natural-language policy induced through a temperature-zero proposer/scorer loop on the HC3 train split, the resulting single-token classifier reaches F0.5 = 0.934 / AUROC = 0.982 / ECE = 0.013 on the HC3 `all` n=4000 test split. That's within ~0.04 F0.5 of a four-times-larger default-prompt baseline and meaningfully better calibrated, which is the part of the answer that's a yes. The qualifier is that the comparison to published baselines is sensitive to which HC3 subset they reported on and which AI source model their corpus was generated from, and that the per-domain breakdown still shows precision deficits on `open_qa` and `wiki_csai` that scale alone doesn't fix. On the explanation side, replacing the induced policy with an adversarially-inverted one flips 49% of test labels and shifts the log-probability margin by 9.6 nats. That's direct behavioural evidence — not a self-explanation that could be plausible-but-wrong in the way Madsen et al. warned about, but a measurement of how much the model's actual decisions depend on the policy content. Taken together: the rule the model uses to flag a text *is* the rule the human reading the explanation will see. That equivalence is what an educator confronting a flagged essay, or an editor handling a flagged submission, or a reviewer adjudicating an authorship dispute, actually needs — not "82% likely AI-generated", but "flagged because of these specific properties, and removing them would change the verdict". The thesis demonstrates that the equivalence is achievable on hardware most people doing this work already own, and that it's measurable in practice rather than just defensible in argument. Whether the resulting explanation is *useful* to a human reader, and whether the policy ports across model families, are the open questions that frame the work that comes next.
