...
|---|---|---|---|---|---|---|---|
| Default prompt (no policy) | Gemma 4 E4B | 1000 | 0.933 | 0.922 | 0.983 | 0.992 | 0.035 |
...
| Default prompt (no policy) | Gemma 4 31B | 1000 | **0.977** | 0.969 | **1.000** | ~1.000 | 0.015 |
| DetectGPT (Mitchell et al. 2023) | — | — | `[TODO: cite HC3 figure from Mitchell et al. 2023 or note non-comparability]` | — | — | — | — |
| Fine-tuned RoBERTa (MGTBench, He et al. 2024) | — | — | `[TODO: cite HC3 figure from He et al. 2024 or note non-comparability]` | — | — | — | — |

Precision and recall on the two default-prompt rows are derived from the recorded F0.5 / F1 and the constraint that recall(AI) = 1.000 on every 31B run (logged in `RUNS.md` notes for the 2026-04-23 31B batch); the predictions JSONLs that produced them are gitignored under `logs/runs/`. The shape of the comparison is what matters: default-prompt E4B sits at recall = 0.983 / precision = 0.922 — the over-predict-AI bias the §4.1 narrative mentions — and 31B catches every single AI text (recall = 1.000) at the cost of slightly lower precision (0.969). Both default-prompt rows are uncalibrated argmax decisions; the calibrated E4B+policy row at the headline trades recall down to 0.898 in exchange for precision rising to 0.943, in the F0.5-favouring direction the use case demands.
...
![Figure 1: ROC + reliability diagram for E4B + induced policy + T2 calibration on the n=4000 HC3 test split. (a) ROC with the F0.5-optimal operating point highlighted (margin = 4.74; precision = 0.944, recall = 0.897; AUROC = 0.982). (b) Reliability — observed accuracy per 10-bin predicted P(AI); ECE = 0.013. Bottom panel shows bin counts on a log scale (the {0.0–0.1} and {0.9–1.0} bins dominate, reflecting the model's near-deterministic verdicts on most examples).](figures/figure-1.png)

**Figure 1.** ROC + reliability diagram for E4B + induced policy + T2 calibration on the n = 4 000 HC3 test split. AUROC = 0.982, ECE = 0.013. Generated from `logs/runs/2026-04-26T19-07-51_137899/features_test.jsonl`.
...
![Figure 2: Policy induction trajectory (run 2026-04-26T17-42-47_3d67db). F0.5 on n = 500 val by iteration; iter 0 = initial seed-pool policy (F0.5 = 0.936); iter 1 = winner (accepted, F0.5 = 0.956); iters 2–6 = identical rejected revisions (refiner deadlock at T = 0); early-stop after 5 consecutive rejections.](figures/figure-2.png)

**Figure 2.** Policy induction trajectory (run `2026-04-26T17-42-47_3d67db`). F0.5 by iteration on n = 500 val; iter 1 winner accepted at F0.5 = 0.956; iters 2–6 rejected by the deterministic refiner; early-stop at iter 6.
...
The headline number is the `best_vs_inverted` row: **49.0% of test examples flip their label** when the system prompt is replaced by the adversarially-inverted policy, and the mean log-probability margin shifts by **+9.6 nats** in the direction the policy content commands (toward `yes` under the best policy, toward `no` under the inverted policy). Per Jacovi and Goldberg [2020] and Lanham et al. [2023], a label-flip rate near zero and a margin shift near zero would mean the policy is a figurehead — the model would be classifying the same way regardless of what the prompt said, and any explanation that pointed at the policy would be plausible-but-not-causal. The observed values are far from zero in both axes, which meets the behavioural-faithfulness criterion for whole-text policy swaps. The `best_vs_empty` deltas are smaller (3% label flip, −0.7 nats) and confirm that the policy and the empty-prompt default agree on most easy cases while diverging on edge cases — i.e. the policy is doing real work on the borderline examples, not the bulk.

![Figure 3: Per-example log-probability margin (logp(yes) − logp(no), in nats) under the best induced policy vs the adversarially-inverted policy on the n = 100 HC3 test slice. Best-policy distribution is bimodal, centred near zero with the two clusters corresponding to correctly-classified human and AI examples. Inverted-policy distribution is shifted strongly toward "no", clustered around −10 nats. Mean shift between best and inverted is +9.56 nats.](figures/figure-3.png)

**Figure 3.** Per-example log-probability margin under best vs adversarially-inverted policies (n = 100 HC3 test). Mean Δ = +9.565 nats; label-flip rate = 0.490. Same-policy consistency (5 re-samples at T = 0) was confirmed identical for all 100 examples in the ablation set. Generated from `logs/runs/2026-04-26T20-23-43_2f80b2/`.
...
**Policy induction vs. OPRO / ProTeGi / DSPy / HtT.** The induction loop in §3.2 is a textbook proposer/scorer/accept-reject loop in the family of OPRO, DSPy/MIPROv2, ProTeGi (Pryzant et al. 2023), TextGrad, Promptbreeder, EvoPrompt, and PromptWizard. The thesis-specific choices are (a) the natural-language *policy* output rather than instruction-tuned exemplars, (b) F0.5 (precision-weighted) as the scoring criterion, and (c) the *retention* of the policy as the explanation artefact post-induction (closer to HtT (Zhu et al. 2024) than to the prompt-search neighbours). A stronger contribution argument would be a head-to-head against ProTeGi or DSPy on the same HC3 splits; this is deferred to Inl. 6 or beyond.

### 5.4 Threats to faithfulness — pre-empting Madsen et al.

Madsen et al. [2024] showed that default-prompted LLM self-explanations are often unfaithful to the model's underlying decision mechanism, and Turpin et al. [2023] earlier showed the same for Chain-of-Thought rationales. The present thesis pre-empts that critique by construction: the policy *is* the system prompt *is* the classifier, so an explanation of the form *"this text was flagged because rule X in the policy applied"* is grounded in the same object that drives the prediction — there is no separate self-explanation step that could diverge. The behavioural ablation in §4.3 supplies the empirical evidence that the policy is causally load-bearing rather than ornamental: a 0.490 label-flip rate and +9.6 nat margin shift between the best and inverted policies is far from the figurehead regime. This defence holds for the *induced* policy explanation only. Any free-text rationalisation generated *alongside* a prediction (e.g. asking the model to justify its `yes`/`no` answer in the same call) would be a separate self-explanation in the Madsen sense and would inherit the unfaithfulness risk; this thesis does not generate such rationalisations.
...
Bao, G., Zhao, Y., Teng, Z., Yang, L., and Zhang, Y. 2024. Fast-DetectGPT: Efficient Zero-Shot Detection of Machine-Generated Text via Conditional Probability Curvature. In *Proceedings of the International Conference on Learning Representations (ICLR)*.

Gehrmann, S., Strobelt, H., and Rush, A. 2019. GLTR: Statistical Detection and Visualization of Generated Text. In *Proceedings of the 57th Annual Meeting of the Association for Computational Linguistics (ACL): System Demonstrations*. 111–116.
...
He, X., Shen, X., Chen, Z., Backes, M., and Zhang, Y. 2024. MGTBench: Benchmarking Machine-Generated Text Detection. In *Proceedings of the 2024 ACM SIGSAC Conference on Computer and Communications Security (CCS)*.
...
Kirchenbauer, J., Geiping, J., Wen, Y., Katz, J., Miers, I., and Goldstein, T. 2023. A Watermark for Large Language Models. In *Proceedings of the 40th International Conference on Machine Learning (ICML)*. PMLR 202, 17061–17084.

Lanham, T., Chen, A., Radhakrishnan, A., Steiner, B., Denison, C., Hernandez, D., Li, D., Durmus, E., Hubinger, E., Kernion, J., Lukošiūtė, K., Nguyen, K., Cheng, N., Joseph, N., Schiefer, N., Rausch, O., McCandlish, S., El Showk, S., Lanham, T., Maxwell, T., Chadwick, T., Yan, T., Brauner, J., Conerly, T., Bowman, S. R., Mann, B., Amodei, D., Olah, C., Askell, A., Hatfield-Dodds, Z., and Perez, E. 2023. Measuring Faithfulness in Chain-of-Thought Reasoning. *arXiv preprint arXiv:2307.13702*.

Madsen, A., Chandar, S., and Reddy, S. 2024. Are Self-Explanations from Large Language Models Faithful? In *Findings of the Association for Computational Linguistics: ACL 2024*. 295–337.

Mitchell, E., Lee, Y., Khazatsky, A., Manning, C. D., and Finn, C. 2023. DetectGPT: Zero-Shot Machine-Generated Text Detection using Probability Curvature. In *Proceedings of the 40th International Conference on Machine Learning (ICML)*. PMLR 202, 24950–24962.
...
Pudasaini, S., Miralles, L., Lillis, D., and Llorens Salvador, M. 2025. Benchmarking AI Text Detection: Assessing Detectors Against New Datasets, Evasion Tactics, and Enhanced LLMs. In *Proceedings of the 1st Workshop on GenAI Content Detection (GenAIDetect) at COLING 2025*.

Turpin, M., Michael, J., Perez, E., and Bowman, S. R. 2023. Language Models Don't Always Say What They Think: Unfaithful Explanations in Chain-of-Thought Prompting. In *Advances in Neural Information Processing Systems (NeurIPS)* 36.

Zhu, Z., Xue, Y., Chen, X., Zhou, D., Tang, J., Schuurmans, D., and Dai, H. 2024. Large Language Models can Learn Rules. In *Proceedings of the International Conference on Learning Representations (ICLR)*. *(Hypotheses-to-Theories / HtT — the closest conceptual match for §3.2: induced rules become the inference prompt.)*
...
- [x] §4.1 Figure 1 (ROC + reliability diagram) — `docs/figures/figure-1.png` from `logs/runs/2026-04-26T19-07-51_137899/`
- [x] §4.2 Figure 2 (induction trajectory) — `docs/figures/figure-2.png` from `logs/runs/2026-04-26T17-42-47_3d67db/`
- [x] §4.3 Figure 3 (margin histogram) — `docs/figures/figure-3.png` from `logs/runs/2026-04-26T20-23-43_2f80b2/`
